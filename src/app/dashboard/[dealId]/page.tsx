...
export default function DealDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Header */}
      <header className="border-b border-zinc-200 px-6 py-3.5 sticky top-0 bg-white/90 backdrop-blur-sm z-10">
        ...
        <div className="grid grid-cols-3 gap-5">
          ...
          <div className="col-span-2 space-y-4">
            ...
            {LOAN_FIELDS.map((section) => (
              <div key={section.section} className="border border-zinc-200 rounded-lg p-4">
                <h2 className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-3">{section.section}</h2>
                    ...
                    return (
                      <div key={field.key} className="space-y-1">
                        <label className="text-[11px] text-zinc-500 flex items-center gap-1">
                          ...
                          {note && (
                            <span className="text-blue-500 cursor-help" title={note}>
                              AI
                            </span>
                          )}
                        </label>
                        <div className="relative">
                          ...
                          <input
                            ...
                            placeholder="—"
                            className={`w-full bg-white border rounded-md px-2.5 py-1.5 text-sm text-zinc-900 placeholder:text-zinc-300 focus:outline-none focus:border-zinc-400 transition-colors ${
                              isMissing ? 'border-amber-300' : 'border-zinc-200'
                            } ${field.prefix ? 'pl-6' : ''}`}
                          />
                          {field.suffix && value && (
            ...
            <div className="border border-zinc-200 rounded-lg p-4">
              ...
              {documents.length === 0 ? (
                ...
                <div className="space-y-1.5">
                  {documents.map((doc) => (
                    <div key={doc.id} className="flex items-start gap-2 p-2 rounded-md bg-zinc-50">
